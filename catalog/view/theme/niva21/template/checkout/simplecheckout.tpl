...
<?php } ?>
    ...
    <?php } ?>
    <div class="simplecheckout-button-block buttons" id="buttons" <?php if ($block_order) { ?>style="display:block;"<?php } ?>>
        <div class="simplecheckout-button-right">
            <?php if ($simple_common_view_agreement_checkbox) { ?><label><input type="checkbox" id="agree" name="agree" value="1" <?php if ($agree == 1) { ?>checked="checked"<?php } ?> /><?php echo $text_agree; ?></label>&nbsp;<?php } ?><a class="button btn" onclick="simplecheckout_submit();" id="simplecheckout_button_confirm"><p class="nova" style="display: inline-block; margin: 0; font-size:14px; padding: 4px 5px 4px 8px;">Заказ подтверждаю</p></a>
        </div>
...
jQuery('#checkout_customer_main_telephone').attr('mask','+38(999)999-9999');
</script>